...
class Results:
    ...
    raw_results : Dict[str, Dict[str, float]]
        All the raw results; first key is the type of the result
        (e.g., MAPE on validation set), second key is the signature
    ...
    def __init__(self, campaign_configuration, exp_confs: List[ec.ExperimentConfiguration]):
        ...
        campaign_configuration: dict of dict:
            The set of options specified by the user though command line
            and campaign configuration files
        ...
        file_handler = logging.FileHandler(os.path.join(
            self._campaign_configuration['General']['output'], 'results'),
            'a+')
    ...
    def collect_data(self):
        ...
        if processes_number == 1:
            ...
            self._exp_confs = list(tqdm.tqdm(pool.imap(evaluate_wrapper, self._exp_confs),
                                             total=len(self._exp_confs)))
            if bool(self._campaign_configuration['General']['generate_plots']):
                pool = multiprocessing.Pool(processes_number)
                self._exp_confs = list(tqdm.tqdm(pool.imap(plot_wrapper, self._exp_confs),
                                                 total=len(self._exp_confs)))
            self._logger.info("<--")
    ...
    def get_bests(self):
        """
        Identify for each considered technique, the configuration with
        the best validation MAPE
        ...
        if (validation, hp_selection) in {("All", "All"),
                                          ("Extrapolation", "All"),
                                          ("All", "HoldOut"),
                                          ("HoldOut", "All"),
                                          ("HoldOut", "HoldOut"),
            ...
            for conf in self._exp_confs:
                ...
                if (technique not in run_tec_best_conf[run]
                        or conf.mapes["hp_selection"] < run_tec_best_conf[run][technique].mapes["hp_selection"]):
            ...
            for run in range(0, self._campaign_configuration['General']['run_num']):
                ...
                for technique in run_tec_best_conf[run]:
                    ...
                    self._logger.info("---Best result for %s - Configuration is %s - "
                                      "(Training MAPE is %f - HP Selection MAPE is %f) - "
                                      "Validation MAPE is %f",
                                      technique, temp.get_signature()[4:],
                                      temp.mapes["training"],
                                      temp.mapes["hp_selection"],
                                      temp.mapes["validation"])

                    # Compute which is the best technique
                    if (not overall_run_best
                            or temp.mapes["hp_selection"] < overall_run_best.mapes["hp_selection"]):
        ...
        elif (validation, hp_selection) in {("KFold", "All"),
                                            ("KFold", "HoldOut")}:
            ...
            for run in range(0, self._campaign_configuration['General']['run_num']):
                ...
                for technique in run_tec_set[run]:
                    self._logger.info("---Best result for %s - (Training MAPE is %f - HP Selection MAPE is %f) - "
                                      "Validation MAPE is %f",
                                      technique, run_tec_set[run][technique]["training"],
                                      run_tec_set[run][technique]["hp_selection"],
                                      run_tec_set[run][technique]["validation"])
            ...
            # Overall best will contain as first argument the technique with the best (across runs)
            # average (across folds) mape on validation; now we consider on all the runs and on all the
            # folds the configuraiton of this technique with best validation mape
            ...
            for run in range(0, self._campaign_configuration['General']['run_num']):
                ...
                for technique in run_tec_best_conf[run]:
                    ...
                    # Compute which is the best technique
                    if (not overall_run_best or temp[1]["hp_selection"]
                            < overall_run_best[2]["hp_selection"]):
            ...
            for conf in self._exp_confs:
                ...
                for set_name in set_names:
                    run_efold_tec_conf_set[run][ext_fold][technique][configuration][set_name] = (run_efold_tec_conf_set[run][ext_fold][technique][configuration][set_name]
                                                                                                 + (conf.mapes[set_name] / (folds * folds)))

            # Select the best configuration for each technique in
            # each external fold across different internal folders
                ...
                for efold in run_efold_tec_conf_set[run]:
                    for tec in run_efold_tec_conf_set[run][efold]:
                        for conf in run_efold_tec_conf_set[run][efold][tec]:
                            if (conf not in run_efold_tec_best_conf[run][efold][tec] or
                                    run_efold_tec_conf_set[run][efold][tec][conf]["hp_selection"]
                                    < run_efold_tec_best_conf[run][efold][tec][1]["hp_selection"]):
                                run_efold_tec_best_conf[run][efold][tec] = (conf,
                                                                            run_efold_tec_conf_set[run][efold][tec][conf],
                                                                            run_efold_tec_conf_set[run][efold][tec][conf])
                    ...
                    for tec in run_efold_tec_best_conf[run][efold]:
                        if "hp_selection" not in run_tec_set[run][tec]:
                            for set_name in set_names:
                                run_tec_set[run][tec][set_name] = 0
                        for set_name in set_names:
                            run_tec_set[run][tec][set_name] = \
                                (run_tec_set[run][tec][set_name]
                                 + run_efold_tec_best_conf[run][efold][tec][1][set_name])
            ...
            for run in range(0, self._campaign_configuration['General']['run_num']):
                ...
                for technique in run_tec_set[run]:
                    ...
                    # Compute which is the best technique
                    if (not overall_run_best or run_tec_set[run][technique]["hp_selection"]
                            < overall_run_best[1]["hp_selection"]):
                        overall_run_best = (technique, run_tec_set[run][technique])
        ...
        for technique in best_confs:
            if (not best_technique or best_confs[technique].mapes["validation"]
                    < best_confs[best_technique].mapes["validation"]):
            ...
            for run in run_tec_conf_set:
                for tec in run_tec_conf_set[run]:
                    for conf in run_tec_conf_set[run][tec]:
                        ...
                        assert "validation" in run_tec_conf_set[run][tec][conf], \
                            ("training MAPE not found for " + str(run) + str(tec) + str(conf))
