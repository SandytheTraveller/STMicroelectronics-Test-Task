...
def main():
    ...
    # My code
    # Adding the argument so that user can specify the list of tests he/she wants to run
    parser.add_argument('-t', "--tests", help="Subset of tests defined by user to run", default=None, type=str)
    ...
    # My code
    # If user provided the subset of tests, then create the list of tests to run
    if args.tests:
        # Split the list where items are separated by comma and
        # Add extension .ini to the name of the test for each item on the list
        chosen_tests = [test.strip() + ".ini" for test in args.tests.split(',')]

    for file in os.listdir(os.path.join(abs_root, "example_configurations")):
        if args.tests and file not in chosen_tests:
            continue
